...
// &bust=f (frame bottom = Head bone - f x (hair top - Head bone), default 0.6).
...
async function render(canvas: HTMLCanvasElement): Promise<string> {
  ...
  const bottom = head.y - (top - head.y) * BUST;
  console.info(`[portrait] #${ID} box y ${box.min.y.toFixed(2)}..${box.max.y.toFixed(2)} head ${head.y.toFixed(2)} (bone ${found})`);
  const midY = (top + bottom) / 2;
  const fov = 24;
  const half = (top - bottom) / 2;
  const dist = half / Math.tan(((fov / 2) * Math.PI) / 180);
  const cam = new PerspectiveCamera(fov, 1, 0.05, 50);
  cam.position.set(head.x + Math.sin(YAW) * dist, midY + 0.08, head.z + Math.cos(YAW) * dist);
  cam.lookAt(head.x, midY, head.z);
