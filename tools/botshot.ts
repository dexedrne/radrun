...
// Default url: ?bot=follow&k=1.3&seed=123&d=chill&c=652&r=4764 on the dev server. Waits for CAUGHT or
...
const predicted = runBotRound(round, { kind: "follow", k: Number(q.get("k") ?? 1.3), yoink: q.get("bot") === "yoink" }, emptyInput());
console.log(`node prediction: ${predicted.caught ? "CAUGHT" : "ESCAPED"} (${predicted.kind || "-"}) at chase step ${predicted.steps} (${predicted.time.toFixed(2)} s)`);
